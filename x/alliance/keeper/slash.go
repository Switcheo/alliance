...
func (k Keeper) SlashValidator(ctx context.Context, valAddr sdk.ValAddress, fraction sdkmath.LegacyDec) error {
	// Slashing must be checked otherwise we can end up slashing incorrect amounts
	if fraction.LTE(sdkmath.LegacyZeroDec()) || fraction.GT(sdkmath.LegacyOneDec()) {
...
func (k Keeper) slashRedelegations(ctx context.Context, valAddr sdk.ValAddress, fraction sdkmath.LegacyDec) error {
...
func (k Keeper) slashUndelegations(ctx context.Context, valAddr sdk.ValAddress, fraction sdkmath.LegacyDec) error {
	sdkCtx := sdk.UnwrapSDKContext(ctx)
	store := k.storeService.OpenKVStore(ctx)
	// Slash all immature re-delegations
