...
func TestSlashingEvent(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		...
		sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(20_000_000)),
	))
	...
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	require.Equal(t, sdkmath.NewInt(20_000_000), tokens)
	...
	totalBondedTokens, err = app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.NotEqual(t, sdkmath.NewInt(13_000_000), totalBondedTokens)
	...
	totalBondedTokens, err = app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(12_999_999), totalBondedTokens)

	// Expect that total tokens with validator 1 are reduced
	val1, _ = app.AllianceKeeper.GetAllianceValidator(ctx, valAddr1)
	asset1, _ = app.AllianceKeeper.GetAssetByDenom(ctx, AllianceDenom)
	tokens = val1.TotalTokensWithAsset(asset1).TruncateInt()
	require.Greater(t, sdkmath.NewInt(20_000_000).Int64(), tokens.Int64())
	asset2, _ = app.AllianceKeeper.GetAssetByDenom(ctx, AllianceDenomTwo)
	tokens = val1.TotalTokensWithAsset(asset2).TruncateInt()
	require.Greater(t, sdkmath.NewInt(20_000_000).Int64(), tokens.Int64())
	...
	require.Less(t, sdkmath.NewInt(20_000_000).Int64(), tokens.Int64())
	asset2, _ = app.AllianceKeeper.GetAssetByDenom(ctx, AllianceDenomTwo)
...
func TestSlashingAfterRedelegation(t *testing.T) {
	...
	_, err = app.AllianceKeeper.Redelegate(ctx, user1, val1, val2, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	require.Equal(t, sdkmath.NewInt(20_000_000), tokens.Amount)
	...
	asset, _ = app.AllianceKeeper.GetAssetByDenom(ctx, AllianceDenom)
	tokens = types.GetDelegationTokens(delegation, val2, asset)
	require.Greater(t, sdkmath.NewInt(20_000_000).Int64(), tokens.Amount.Int64())
...
func TestSlashingAfterUndelegation(t *testing.T) {
	...
	_, err = app.AllianceKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	unbondingTime, err := app.StakingKeeper.UnbondingTime(ctx)
	require.NoError(t, err)
	undelegationsIter, err := app.AllianceKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime().Add(unbondingTime).Add(time.Second))
	require.NoError(t, err)
	...
	require.Greater(t, sdkmath.NewInt(10_000_000).Int64(), entry.Balance.Amount.Int64())
	...
	// Expect that delegation stayed the same
	undelegationsIter, err = app.AllianceKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime())
	require.NoError(t, err)
	require.True(t, undelegationsIter.Valid())
...
func TestSlashingIncorrectAmount(t *testing.T) {
	...
	err := app.AllianceKeeper.SlashValidator(ctx, sdk.ValAddress(addrs[0]), sdkmath.LegacyNewDec(2))
	require.EqualErrorf(t, err, "slashed fraction must be greater than 0 and less than or equal to 1: 2.000000000000000000", "")

	err = app.AllianceKeeper.SlashValidator(ctx, sdk.ValAddress(addrs[0]), sdkmath.LegacyNewDec(-1))
