...
func TestDelegationWithASingleAsset(t *testing.T) {
	...
	app.AllianceKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.AllianceAsset{
			...
			types.NewAllianceAsset(AllianceDenomTwo, sdkmath.LegacyNewDec(10), sdkmath.LegacyNewDec(2), sdkmath.LegacyNewDec(12), sdkmath.LegacyNewDec(0), genesisTime),
		},
	})
	...
	require.Equal(t, totalBonded.Mul(sdkmath.NewInt(2)).String(), allianceBonded.String())
	...
	require.Equal(t, types.AllianceAsset{
		...
		TotalTokens:          sdkmath.NewInt(1000_000),
		TotalValidatorShares: sdkmath.LegacyNewDec(1000_000),
	...
	require.Equal(t, types.Delegation{
		...
		ValidatorAddress: val.OperatorAddress,
		Denom:            AllianceDenom,
		Shares:           sdkmath.LegacyNewDec(2000_000),
		RewardHistory:    types.RewardHistories(nil),
	}, allianceDelegation)
	...
	require.Equal(t, types.AllianceAsset{
		...
		RewardWeightRange:    types.RewardWeightRange{Min: sdkmath.LegacyNewDec(0), Max: sdkmath.LegacyNewDec(5)},
		TakeRate:             sdkmath.LegacyNewDec(0),
		TotalTokens:          sdkmath.NewInt(2000_000),
		TotalValidatorShares: sdkmath.LegacyNewDec(2000_000),
		RewardStartTime:      genesisTime,
		RewardChangeRate:     sdkmath.LegacyOneDec(),
		RewardChangeInterval: 0,
	}, asset)
...
func TestDelegationWithMultipleAssets(t *testing.T) {
	...
	app.AllianceKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.AllianceAsset{
			types.NewAllianceAsset(AllianceDenom, sdkmath.LegacyNewDec(2), sdkmath.LegacyZeroDec(), sdkmath.LegacyNewDec(5), sdkmath.LegacyNewDec(0), ctx.BlockTime()),
	...
	// Delegate
	_, err = app.AllianceKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(2000_000)))
	require.NoError(t, err)
	// Delegate with another denom
	_, err = app.AllianceKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(1000_000)))
	require.NoError(t, err)
	...
	require.Equal(t, stakingtypes.Delegation{
		DelegatorAddress: moduleAddr.String(),
		ValidatorAddress: valAddr.String(),
		Shares:           sdkmath.LegacyNewDec(12),
	}, newDelegation)

	// Check validator in x/staking
	val, err = app.AllianceKeeper.GetAllianceValidator(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(13), val.DelegatorShares)
}
...
func TestDelegationWithUnknownAssets(t *testing.T) {
	...
	// Mint alliance tokens
	err = app.BankKeeper.MintCoins(ctx, minttypes.ModuleName, sdk.NewCoins(sdk.NewCoin("UNKNOWN", sdkmath.NewInt(2000_000))))
	require.NoError(t, err)

	_, err = app.AllianceKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin("UNKNOWN", sdkmath.NewInt(2000_000)))
	require.Error(t, err)
}
...
func TestSuccessfulRedelegation(t *testing.T) {
	...
	err = app.BankKeeper.MintCoins(ctx, minttypes.ModuleName, sdk.NewCoins(sdk.NewCoin(AllianceDenom, sdkmath.NewInt(2000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr1, sdk.NewCoins(sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr2, sdk.NewCoins(sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000))))
	require.NoError(t, err)

	// First delegate to validator 1
	_, err = app.AllianceKeeper.Delegate(ctx, delAddr1, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)))
	require.NoError(t, err)

	// Then redelegate to validator2
	_, err = app.AllianceKeeper.Redelegate(ctx, delAddr1, val1, val2, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)))
	require.NoError(t, err)
	...
	for _, d := range delegations {
		if d.DelegatorAddress == moduleAddr.String() {
			require.Equal(t, stakingtypes.Delegation{
				...
				Shares:           sdkmath.LegacyNewDec(2_000_000),
			}, d)
		}
	}
	...
	for ; iter.Valid(); iter.Next() {
		...
		require.Equal(t, types.Redelegation{
			...
			Balance:             sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)),
		}, redelegation)
	}
	...
	require.Equal(t, types.Delegation{
		...
		Denom:            AllianceDenom,
		Shares:           sdkmath.LegacyNewDec(500_000),
		RewardHistory:    types.RewardHistories(nil),
	}, dstDelegation)
	require.True(t, found)
	...
	_, err = app.AllianceKeeper.Delegate(ctx, delAddr2, val2, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)))
	require.NoError(t, err)
	...
	_, err = app.AllianceKeeper.Redelegate(ctx, delAddr2, val2, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)))
	...
	deleted, err := app.AllianceKeeper.CompleteRedelegations(ctx)
	require.NoError(t, err)
...
func TestRedelegationFailsWithTransitiveDelegation(t *testing.T) {
	...
	_, err = app.AllianceKeeper.Redelegate(ctx, delAddr1, val2, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(500_000)))
	require.Error(t, err)
}
...
func TestRedelegationFailsWithGreaterAmount(t *testing.T) {
	...
	_, err = app.AllianceKeeper.Redelegate(ctx, delAddr2, val2, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000)))
	require.Error(t, err)
}
...
func TestSuccessfulUndelegation(t *testing.T) {
	...
	require.Equal(t, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000)), coin)
	...
	_, err = app.AllianceKeeper.Undelegate(ctx, delAddr, val, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(250_000)))
	require.NoError(t, err)
	...
	iter, err := app.AllianceKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime().Add(unbondingTime).Add(time.Second))
	require.NoError(t, err)
	require.True(t, iter.Valid())
	...
	require.Equal(t, types.QueuedUndelegation{Entries: []*types.Undelegation{
		{
			...
			Balance:          sdk.NewCoin(AllianceDenom, sdkmath.NewInt(250_000)),
		},
		{
			DelegatorAddress: delAddr.String(),
			ValidatorAddress: val.GetOperator(),
	...
	coin = app.BankKeeper.GetBalance(ctx, delAddr, AllianceDenom)
	require.Equal(t, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1500_000)), coin)
...
func TestUndelegationWithoutDelegation(t *testing.T) {
	...
	_, err = app.AllianceKeeper.Undelegate(ctx, delAddr, val, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000)))
	require.Error(t, err)
}
...
func TestUndelegateAfterClaimingTakeRate(t *testing.T) {
	...
	app.AllianceKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.AllianceAsset{
			types.NewAllianceAsset(AllianceDenom, sdkmath.LegacyNewDec(2), sdkmath.LegacyZeroDec(), sdkmath.LegacyNewDec(5), sdkmath.LegacyNewDec(0), ctx.BlockTime()),
			types.NewAllianceAsset(AllianceDenomTwo, sdkmath.LegacyNewDec(10), sdkmath.LegacyNewDec(2), sdkmath.LegacyNewDec(12), sdkmath.LegacyMustNewDecFromStr("0.5"), ctx.BlockTime()),
		},
	})
	queryServer := keeper.NewQueryServerImpl(app.AllianceKeeper)
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		...
		sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(2000_000_000)),
	))
	...
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(1000_000_000)))
	require.NoError(t, err)
	...
	totalBondedTokens, err := app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(11_000_000), totalBondedTokens)
	...
	require.True(t, del.GetBalance().Amount.LT(sdkmath.NewInt(1000_000_000)), "%s should be less than %s", del.GetBalance().Amount, sdkmath.NewInt(1000_000_000))
	// Undelegate token with initial amount should fail
	_, err = app.AllianceKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(1000_000_000)))
	require.Error(t, err)
	...
	// Delegate again
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(500_000_000)))
	require.NoError(t, err)

	ctx = ctx.WithBlockTime(ctx.BlockTime().Add(time.Minute * 1)).WithBlockHeight(2)

	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(400_000_000)))
	require.NoError(t, err)
...
func TestDelegationWithNativeStakingChanges(t *testing.T) {
	...
	require.NoError(t, err)
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(bondDenom, sdkmath.NewInt(1000_000_000)),
		sdk.NewCoin(AllianceDenom, sdkmath.NewInt(1000_000_000)),
		sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(2000_000_000)),
	))
	pks := test_helpers.CreateTestPubKeys(2)
	...
	_, err = app.AllianceKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(AllianceDenom, sdkmath.NewInt(2000_000)))
	require.NoError(t, err)
	_, err = app.AllianceKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(AllianceDenomTwo, sdkmath.NewInt(2000_000)))
	require.NoError(t, err)
	assets := app.AllianceKeeper.GetAllAssets(ctx)
	err = app.AllianceKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	_, err = app.StakingKeeper.Delegate(ctx, user2, sdkmath.NewInt(2000_000), stakingtypes.Unbonded, *val2.Validator, true)
	require.NoError(t, err)
	// Check total bonded tokens
	totalBondedTokens, err = app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(15_000_000), totalBondedTokens)
	...
	totalBondedTokens, err = app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(39_000_000), totalBondedTokens)

	// Redelegate some native tokens
	_, err = app.StakingKeeper.BeginRedelegation(ctx, user2, valAddr2, valAddr1, sdkmath.LegacyNewDec(1))
	require.NoError(t, err)
	...
	shares, _ := app.StakingKeeper.ValidateUnbondAmount(ctx, user2, valAddr2, sdkmath.NewInt(1000_000))
	...
	totalBondedTokens, err = app.StakingKeeper.TotalBondedTokens(ctx)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(26_000_000), totalBondedTokens)
}
