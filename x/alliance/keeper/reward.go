...
func accumulateRewards(latestRewardHistories types.RewardHistories, rewardHistories types.RewardHistories, asset types.AllianceAsset, rewardWeight sdkmath.LegacyDec, delegation types.Delegation, validator types.AllianceValidator) (sdk.Coins, types.RewardHistories) {
	// Go through each reward denom and accumulate rewards
	var rewards sdk.Coins

	delegationTokens := sdkmath.LegacyNewDecFromInt(types.GetDelegationTokens(delegation, validator, asset).Amount)
	for _, history := range latestRewardHistories {
		...
		if !found {
			...
			rewardHistory.Index = sdkmath.LegacyZeroDec()
		}
	...
	for _, c := range coins {
		...
		if !found {
			rewardHistories = append(rewardHistories, types.RewardHistory{
				Denom: c.Denom,
				Index: sdkmath.LegacyNewDecFromInt(c.Amount).Quo(totalAssetWeight),
			})
		} else {
			rewardHistory.Index = rewardHistory.Index.Add(sdkmath.LegacyNewDecFromInt(c.Amount).Quo(totalAssetWeight))
		}
	}
...
func (k Keeper) totalAssetWeight(ctx context.Context, val types.AllianceValidator) sdkmath.LegacyDec {
	sdkCtx := sdk.UnwrapSDKContext(ctx)
